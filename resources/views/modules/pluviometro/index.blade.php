            ...
            <div class="container-fluid" bis_skin_checked="1">
                <div class="card" bis_skin_checked="1">
                    <div class="card-header" bis_skin_checked="1">
                        <h3 class="card-title">Listado de pluviómetros</h3>
                    ...
                    <div class="card-body" bis_skin_checked="1">
                        <div class="d-flex gap-2 mb-3" bis_skin_checked="1">
                            ...
                            <a href="{{ route('pluviometria.index') }}" type="button" class="btn btn-sm btn-outline-primary">
                                <i class="bi bi-moisture me-1" aria-hidden="true"></i>
                                Pluviometria
                            </a>
                            <a href="{{ route('pluviometros.create') }}" type="button"
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                @forelse ($pluviometros as $pluviometro)
                                    <tr>
                                        <th scope="row">{{ $pluviometro->id }}</th>
                                        <td>{{ $pluviometro->nombre }}</td>
                                        <td>{{ $pluviometro->ubicacion }}</td>
                                        <td>{{ $pluviometro->observaciones }}</td>
                                        <td>
                                            <div class="btn-group" role="group">
                                                ...
                                                <form action="{{ route('pluviometros.destroy', $pluviometro->id) }}" method="POST"
                                                    class="d-inline">
                                                    @csrf
                                                    @method('DELETE')

                                                    <button type="submit" class="btn btn-sm btn-outline-danger"
                                                        onclick="return confirm('¿Estás completamente seguro de eliminar el pluviometro {{ $pluviometro->nombre }}? Esta acción no se puede deshacer.');">
